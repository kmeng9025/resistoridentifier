...
# =========================
def main():
    frame = cv2.imread("brownblackblackgoldbrown.png")
    out = process_frame(frame, frame_idx=0)
    if out is not None:
        # Print compact band results
        bands = out["bands"]
        # Example output: label(conf) label(conf) ...
    ...
    #         # Example output: label(conf) label(conf) ...
    #         summary = " | ".join([f"{lbl}({conf:.2f})" for (lbl, conf, _, _) in bands])
    #         print(f"[{frame_idx}] bands: {summary}")

    #     frame_idx += 1

    #     # sleep until next 0.5s tick
    #     next_time += FRAME_INTERVAL_SEC
    #     sleep_for = next_time - time.time()
    #     if sleep_for > 0:
    #         time.sleep(sleep_for)
    #     else:
    #         # if we fell behind, reset schedule to avoid runaway lag
    #         next_time = time.time()
